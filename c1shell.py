...
catnames = ['EOF', 'PRINT', 'UNSIGNEDINT', 'NAME', 'ASSIGNOP',
            ...
            'TIMES', 'NEWLINE', 'ERROR','POWER', 'DIV', 'IF', 'GT', 'LT', 'BEGINBLOCK', 'ENDBLOCK','STRING']
#Se agregaron mas nombres arribas

# keywords and their token categories}
keywords = {'print': PRINT, 'if': IF}
#Se agrego If aca arriba
...
def tokenizer():
   ...
   while True:
      ...
      elif curchar == '"':  # start of string literal
         token.category = STRING
         token.lexeme += curchar  # opening "
         curchar = getchar()
         while curchar != '"' and curchar != '':
            token.lexeme += curchar
            curchar = getchar()
         if curchar == '"':
            token.lexeme += curchar  # closing "
            curchar = getchar()
         else:
...
def condition():
   if token.category in [UNSIGNEDINT, NAME]:
      ...
      raise RuntimeError("Expecting unsigned int or name in condition")
...
def comparator():
   if token.category in (LT,GT):
      advance()
   else:
      raise RuntimeError("Expecting comparator...")
